...
class ResultCard(tk.Frame):
    def __init__(self, parent, label, **kw):
        ...
        tk.Label(self, text=label, font=(FONT,8), fg=TEXT_SEC, bg=BG_FIELD).pack(pady=10)
...
class App(tk.Tk):
    ...
    def _header(self):
        ...
        logo.pack(side="left", padx=18, pady=12)
    ...
    def _field(self, p, label):
        ...
        tk.Label(f, text=label, font=(FONT,8), fg=TEXT_SEC, bg=BG_CARD).pack(anchor="w", pady=3)
        return f
    def _card(self, p):
    ...
    def _build_raid(self):
        ...
        left = self._card(p); left.grid(row=0,column=0,sticky="nsew",padx=8)
        tk.Label(left, text="Configuração de Discos", font=(FONT,11,"bold"),
                 fg=ACCENT, bg=BG_CARD).pack(anchor="w", padx=14, pady=12)
        ...
        self.lbl_rdesc.pack(anchor="w", pady=6)
        ...
        self.rc_rd = ResultCard(right,"HDs no Array"); self.rc_rd.pack(fill="x",pady=6)
        self.rc_rb = ResultCard(right,"Capacidade Bruta"); self.rc_rb.pack(fill="x",pady=6)
        self.rc_ru = ResultCard(right,"Volume Disponível"); self.rc_ru.pack(fill="x",pady=6)
        self.rc_re = ResultCard(right,"Eficiência"); self.rc_re.pack(fill="x",pady=6)
    ...
    def _build_cam(self):
        ...
        c1 = tk.Frame(r2, bg=BG_CARD); c1.pack(side="left",expand=True,fill="x",padx=6)
        tk.Label(c1,text="Dias de retenção",font=(FONT,8),fg=TEXT_SEC,bg=BG_CARD).pack(anchor="w",pady=3)
        ...
        tk.Label(c2,text="Horas/dia gravando",font=(FONT,8),fg=TEXT_SEC,bg=BG_CARD).pack(anchor="w",pady=3)
        ...
        right = tk.Frame(p, bg=BG_DARK); right.grid(row=0,column=1,sticky="nsew")
        self.rc_cbt = ResultCard(right,"Bitrate Total"); self.rc_cbt.pack(fill="x",pady=6)
        self.rc_cpd = ResultCard(right,"Por câmera / dia"); self.rc_cpd.pack(fill="x",pady=6)
        self.rc_cst = ResultCard(right,"Armazenamento Necessário"); self.rc_cst.pack(fill="x",pady=6)
        self.rc_cd1 = ResultCard(right,"Dias com 1 TB"); self.rc_cd1.pack(fill="x",pady=6)
    ...
    def _build_combined(self):
        ...
        tk.Label(p, text="Análise Combinada — RAID vs Necessidade de Armazenamento",
                 font=(FONT,11,"bold"), fg=TEXT_PRI, bg=BG_DARK).pack(anchor="w", pady=10)

        row = tk.Frame(p, bg=BG_DARK); row.pack(fill="x")
        self.rc_xr = ResultCard(row,"Disponível no RAID")
        self.rc_xr.pack(side="left",expand=True,fill="x",padx=6)
        self.rc_xc = ResultCard(row,"Necessário pelas Câmeras")
        self.rc_xc.pack(side="left",expand=True,fill="x",padx=6)
        ...
        self.lbl_vsub = tk.Label(self.vrd_frame, text="", font=(FONT,9),
                                  fg=TEXT_SEC, bg=BG_FIELD, padx=14, pady=10,
                                  wraplength=760, justify="left")
        self.lbl_vsub.pack(anchor="w")

        gf = self._card(p); gf.pack(fill="x")
        tk.Label(gf, text="Utilização do Storage", font=(FONT,9), fg=TEXT_SEC, bg=BG_CARD).pack(anchor="w",padx=14,pady=10)
        bg_bar = tk.Frame(gf, bg=BORDER, height=24); bg_bar.pack(fill="x",padx=14,pady=4)
        ...
        self.lbl_pct.pack(anchor="e",padx=14,pady=10)
